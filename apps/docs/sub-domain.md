Yes, absolutely! Using a subdomain like https://anoda.otagera.xyz/ is the industry-standard way to host a project like this.
   ...
   * Frontend: https://anoda.otagera.xyz
   * API: https://api.anoda.otagera.xyz
  ...
  Log into your domain provider (e.g., Cloudflare, Namecheap, Google Domains) and add these records:
   1. A Record: anoda → Point to your VPS IP Address.
   2. A Record: api.anoda → Point to the same VPS IP Address.

  B. Coolify Configuration
  In the Coolify dashboard, for each application:
   * Client (Static Site): Set the FQDN (Fully Qualified Domain Name) field to https://anoda.otagera.xyz.
   * API (Docker Compose): Set the FQDN field for the API service to https://api.anoda.otagera.xyz.
   ...
   * VITE_API_URL for the Client should be https://api.anoda.otagera.xyz.
   * BASE_API_URL for the API should be https://api.anoda.otagera.xyz.
