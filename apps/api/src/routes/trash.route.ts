...
const trashRoutes = new Elysia({ prefix: "/trash" })
	...
	.post("/images/restore", async ({ body, userId, set }) => {
		try {
			const data = await restorePictureService({
				...body,
				userId,
			});
			return { status: "completed", message: "Images restored successfully", data };
		} catch (error: any) {
			set.status = HTTP_STATUS_CODES.BAD_REQUEST;
			return { status: "error", message: error.message };
		}
	...
	.post("/albums/:albumId/restore", async ({ params, userId, set }) => {
		try {
			const data = await restoreAlbumService({
				albumId: params.albumId,
				userId,
			});
			return { status: "completed", message: "Album restored successfully", data };
		} catch (error: any) {
			set.status = HTTP_STATUS_CODES.BAD_REQUEST;
			return { status: "error", message: error.message };
		}
