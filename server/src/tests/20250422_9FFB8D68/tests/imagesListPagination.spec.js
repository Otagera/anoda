  ...
  describe("GET /images (List, Filter, Paginate)", () => {
    ...
    describe("Offset Pagination", () => {
      ...
      test("GET /images - should paginate correctly (limit=2, page=2)", async () => {
        ...
        const sortedImagesDataList = [...imagesDataList].sort((a, b) =>
          b.image_id > a.image_id ? 1 : -1
        );
        expect(res.body.data.images[0].imageId).toBe(
          sortedImagesDataList[expectedStartIndex].image_id
        );
        expect(res.body.data.images[1].imageId).toBe(
          sortedImagesDataList[expectedStartIndex + 1].image_id
        );
      });
    ...
    describe("Cursor Pagination", () => {
      ...
      test("GET /images - should list first page and return nextCursor", async () => {
        ...
        const sortedImages = [...imagesDataList].sort((a, b) =>
          b.image_id > a.image_id ? 1 : -1
        );

        for (let i = 0; i < limit; i++) {
          expect(res.body.data.images[i].imageId).toBe(
            sortedImages[i].image_id
          );
        }
      });
      ...
      test("GET /images - should use nextCursor to get the second page", async () => {
        ...
        expect(secondPageIds[0]).toBe(sortedImagesDataList[limit].image_id); // First item after the first page
        expect(secondPageIds[limit - 1]).toBe(
          sortedImagesDataList[limit * 2 - 1].image_id
        );
      });
      ...
      test("GET /images - should reach the end of the list (nextCursor is null)", async () => {
        ...
        expect(
          secondRes.body.data.images[secondRes.body.data.images.length - 1]
            .imageId
        ).toBe(sortedImagesDataList[NUM_IMAGES_LIST - 1].image_id);
      });
      ...
      test("GET /images - should filter by albumId and paginate with cursor", async () => {
        ...
        const secondRes = await agent
          .get(
            `${baseURL}/images?paginationType=cursor&albumId=${listAlbumId1}&limit=${limit}&nextCursor=${nextCursor}`
          )
          .set("Authorization", `Bearer ${authToken}`);
        ...
        const originalData = imagesDataList.find(
          (d) => d.image_id === secondRes.body.data.images[0].imageId
        );
        expect(originalData.assignedAlbumId).toBe(listAlbumId1);
      });
      ...
      test("GET /images - should return 400 for invalid cursor format", async () => {
        ...
        const res = await agent
          .get(
            `${baseURL}/images?paginationType=cursor&limit=${limit}&nextCursor=${invalidCursor}`
          )
          .set("Authorization", `Bearer ${authToken}`);
      ...
      test("GET /images - should return 400 for invalid prevCursor format", async () => {
        const limit = 3;
        const invalidPrevCursor = "this-is-not-a-valid-cursor";
        const res = await agent
          .get(
            `${baseURL}/images?paginationType=cursor&limit=${limit}&prevCursor=${invalidPrevCursor}`
          )
          .set("Authorization", `Bearer ${authToken}`);
