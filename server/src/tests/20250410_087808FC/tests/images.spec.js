  ...
  beforeAll(async () => {
      ...
      if (!testAlbumId) throw new Error ("Test album ID not available for album-image tests");
       ...
       if (createRes1.status !== HTTP_STATUS_CODES.CREATED || !createRes1.body.data.image_id ||
           ...
           throw new Error("Failed to create images in beforeAll for album-image tests");
       }
       testImageId1 = createRes1.body.data.image_id;
       testImageId2 = createRes2.body.data.image_id;
  });
   ...
   describe("POST /albums/:albumId/images", () => {
       test("should add an existing image to an album", async () => {
           const res = await agent
               ...
               .set("Authorization", `Bearer ${authToken}`)
               .send({ imageId: testImageId1 }); // Send image ID in body

           expect(res.status).toBe(HTTP_STATUS_CODES.CREATED); // Or OK (200) depending on design
           expect(res.body.status).toBe("completed");
           expect(res.body.message).toBe("Image added to album successfully.");
           // Optionally check response data if it includes link ID or confirmation
       });
       ...
       test("should fail if album does not exist", async () => {
        ...
        test("should fail if imageId is missing in request body", async () => {
            const res = await agent
               ...
               .send({}); // Empty body
            expect(res.status).toBe(HTTP_STATUS_CODES.BAD_REQUEST);
             expect(res.body.message).toMatch(/imageId is required/i);
       });
    ...
    describe("GET /albums/:albumId/images", () => {
        beforeEach(async () => {
             // Add both images to the test album for listing
             await agent.post(`${baseURL}/albums/${testAlbumId}/images`).set("Authorization", `Bearer ${authToken}`).send({ imageId: testImageId1 });
             await agent.post(`${baseURL}/albums/${testAlbumId}/images`).set("Authorization", `Bearer ${authToken}`).send({ imageId: testImageId2 });
        });
        ...
        test("should list images in a specific album", async () => {
             ...
             const imageIds = res.body.data.images.map(img => img.image_id);
             expect(imageIds).toContain(testImageId1);
             expect(imageIds).toContain(testImageId2);
             expect(res.body.data.images[0]).toHaveProperty("image_path");
             expect(res.body.data.images[0]).toHaveProperty("original_width");
        });

         test("should return an empty array if album has no images", async () => {
             // Clear the links added in beforeEach
    ...
    describe("DELETE /albums/:albumId/images/:imageId", () => {
         ...
         test("should remove an image from an album", async () => {
             ...
             // Verify removal
             const getRes = await agent.get(`${baseURL}/albums/${testAlbumId}/images`).set("Authorization", `Bearer ${authToken}`);
             expect(getRes.body.data.images.length).toBe(0); // Assumes only this image was linked
         });

         test("should fail without authentication", async () => {
             const res = await agent.delete(`${baseURL}/albums/${testAlbumId}/images/${testImageId1}`);
             expect(res.status).toBe(HTTP_STATUS_CODES.UNAUTHORIZED);
         });
         ...
         test("should return 404 if album does not exist", async () => {
             const nonExistentAlbumId = "123e4567-e89b-12d3-a456-426614174000";
              const res = await agent
                .delete(`${baseURL}/albums/${nonExistentAlbumId}/images/${testImageId1}`)
                .set("Authorization", `Bearer ${authToken}`);
             expect(res.status).toBe(HTTP_STATUS_CODES.NOTFOUND);
         ...
         test("should return 404 if image does not exist", async () => {
          ...
          test("should return 404 if image exists but is not in the specified album", async () => {
             ...
             const res = await agent
                ...
                .set("Authorization", `Bearer ${authToken}`);
             expect(res.status).toBe(HTTP_STATUS_CODES.NOTFOUND);
             expect(res.body.message).toMatch(/Image not found in album|Link not found/i); // Adjust message
         });
    });
}); */
...
/* describe("Faces Endpoints", () => {
    let testImageId;
    let testFaceId1; // Assume face detection/creation happens elsewhere or is mocked
    ...
    beforeAll(async () => {
        ...
        if (Faces && Faces.createFace) {
             const faceData1 = {
                image_id: testImageId,
                embedding: [0.1, 0.2, 0.3], // Example embedding
                bounding_box: { x: 10, y: 20, w: 50, h: 60 } // Example bbox JSON
             };
             const createdFace = await Faces.createFace(faceData1); // Use the helper
             testFaceId1 = createdFace.face_id; // Get the auto-incremented ID

             // Create another face for the same image if needed for list tests
             // const faceData2 = { ... }; await Faces.createFace(faceData2);
        } else {
            throw new Error("Faces helper or createFace function is missing for test setup.");
        }
         if (!testFaceId1) {
    ...
    describe("GET /images/:imageId/faces", () => {
        test("should list faces for a specific image", async () => {
            ...
            expect(res.body.data.faces.length).toBeGreaterThanOrEqual(1); // Should find the face(s) we created

            const face = res.body.data.faces.find(f => f.face_id === testFaceId1);
            expect(face).toBeDefined();
            expect(face).toHaveProperty("image_id", testImageId);
            expect(face).toHaveProperty("embedding"); // Check if embedding is returned
            expect(face).toHaveProperty("bounding_box");
            expect(face.bounding_box).toEqual({ x: 10, y: 20, w: 50, h: 60 });
        });
    ...
    describe("Advanced Image and Face Endpoint Tests", () => {
      ...
      describe("GET /images (List, Filter, Paginate)", () => {
          ...
          beforeAll(async () => {
              ...
              if (!Albums || !Albums.createAlbum || !Images || !Images.createImage || !AlbumImages || !AlbumImages.linkImageToAlbum) {
                   throw new Error("Required helper functions (Albums/Images/AlbumImages) are missing for list test setup.");
              }
  
              // Create two albums
              // Ensure testUserId is defined from a higher scope or fetched here
              if (!global.testUserId) { // Example check if testUserId is globally available
              ...
              const albumData1 = { albumName: "List Test Album One", userId: global.testUserId };
              const albumData2 = { albumName: "List Test Album Two", userId: global.testUserId };
              ...
              if (!listAlbumId1 || !listAlbumId2) throw new Error("Failed to create albums for listing tests.");
              ...
              for (let i = 0; i < NUM_IMAGES_LIST; i++) {
                  ...
                  if (i < 3) { // First 3 in Album 1
                      assignedAlbumId = listAlbumId1;
                  } else if (i < 5) { // Next 2 in Album 2
                  ...
                  imagesDataList.push({ image_id: imgId, assignedAlbumId: assignedAlbumId });
          ...
          afterAll(async () => {
              ...
              for (const img of imagesDataList) {
                  if (AlbumImages?.deleteLinksByImageId) await AlbumImages.deleteLinksByImageId(img.image_id);
          ...
          describe("Offset Pagination", () => {
              test("GET /images - should list first page with default limit", async () => {
                  ...
                  expect(res.body.data.images.length).toBeLessThanOrEqual(NUM_IMAGES_LIST);
              ...
              test("GET /images - should paginate correctly (limit=2, page=2)", async () => {
                  ...
                  expect(res.body.data.pagination.totalPages).toBe(Math.ceil(NUM_IMAGES_LIST / limit));
  
                  // Verify correct items are returned (indices 2 and 3 assuming default sort order)
                  const expectedStartIndex = (page - 1) * limit;
                  expect(res.body.data.images[0].image_id).toBe(imagesDataList[expectedStartIndex].image_id);
                  expect(res.body.data.images[1].image_id).toBe(imagesDataList[expectedStartIndex + 1].image_id);
              });
              ...
              test("GET /images - should filter by albumId and paginate (offset)", async () => {
                  ...
                  const page = 1;
                  const res = await agent
                      .get(`${baseURL}/images?albumId=${listAlbumId1}&limit=${limit}&page=${page}`)
                      .set("Authorization", `Bearer ${authToken}`);
  
                  const expectedTotalFiltered = imagesDataList.filter(img => img.assignedAlbumId === listAlbumId1).length; // 3
                  expect(res.status).toBe(HTTP_STATUS_CODES.OK);
                  ...
                  expect(res.body.data.pagination.page).toBe(page);
                  expect(res.body.data.pagination.limit).toBe(limit);
                  expect(res.body.data.pagination.totalPages).toBe(Math.ceil(expectedTotalFiltered / limit)); // 2 pages total
              });
          ...
          describe("Cursor Pagination", () => {
              ...
              test("GET /images - should list first page and return nextCursor", async () => {
                  ...
                  expect(res.body.data.images[0].image_id).toBe(imagesDataList[0].image_id);
                  expect(res.body.data.images[limit - 1].image_id).toBe(imagesDataList[limit - 1].image_id);
              });
              ...
              test("GET /images - should use nextCursor to get the second page", async () => {
                  ...
                  const firstPageIds = firstRes.body.data.images.map(img => img.image_id);
                  const secondPageIds = secondRes.body.data.images.map(img => img.image_id);
                  ...
                  expect(secondPageIds[limit - 1]).toBe(imagesDataList[limit * 2 - 1].image_id);
              });
               ...
               test("GET /images - should reach the end of the list (nextCursor is null)", async () => {
                  ...
                  // Page 1
                  const firstRes = await agent.get(`${baseURL}/images?limit=${limit}`).set("Authorization", `Bearer ${authToken}`);
                  ...
                  const secondRes = await agent.get(`${baseURL}/images?limit=${limit}&cursor=${cursor1}`).set("Authorization", `Bearer ${authToken}`);
                  expect(secondRes.status).toBe(HTTP_STATUS_CODES.OK);
                  ...
                  expect(secondRes.body.data.images[secondRes.body.data.images.length - 1].image_id)
                      .toBe(imagesDataList[NUM_IMAGES_LIST - 1].image_id);
              });
  
              test("GET /images - should filter by albumId and paginate with cursor", async () => {
                  const limit = 2;
                  const expectedFilteredCount = imagesDataList.filter(img => img.assignedAlbumId === listAlbumId1).length; // 3
                  ...
                  firstRes.body.data.images.forEach(img => {
                      const originalData = imagesDataList.find(d => d.image_id === img.image_id);
                      expect(originalData.assignedAlbumId).toBe(listAlbumId1);
                  });
  
                   // Page 2 of filtered results
                   const secondRes = await agent
                      .get(`${baseURL}/images?albumId=${listAlbumId1}&limit=${limit}&cursor=${nextCursor}`)
                      .set("Authorization", `Bearer ${authToken}`);
  
                  expect(secondRes.status).toBe(HTTP_STATUS_CODES.OK);
                   // Should contain the remaining item(s)
                  expect(secondRes.body.data.images.length).toBe(expectedFilteredCount - limit); // 1 item
                  expect(secondRes.body.data.pagination.nextCursor).toBeNull(); // End of filtered list
  
                  // Verify the last item belongs to the album
                   const originalData = imagesDataList.find(d => d.image_id === secondRes.body.data.images[0].image_id);
                   expect(originalData.assignedAlbumId).toBe(listAlbumId1);
              });
          ...
          test("GET /images - should filter images by albumId (basic check)", async () => {
              ...
              const expectedCount = imagesDataList.filter(img => img.assignedAlbumId === listAlbumId1).length;
              expect(res.status).toBe(HTTP_STATUS_CODES.OK);
      ...
      describe("Faces Endpoints (Get Specific / Search)", () => {
          ...
          const embeddingP1_1 = [0.10, 0.20, 0.10]; // Person 1, Face 1
          const embeddingP1_2 = [0.11, 0.21, 0.11]; // Person 1, Face 2 (similar)
          const embeddingP2_1 = [0.80, 0.10, 0.30]; // Person 2, Face 1 (different)
          const embeddingP3_1 = [0.50, 0.50, 0.90]; // Person 3, Face 1 (different)
          ...
          beforeAll(async () => {
              // Ensure helpers are available
              if (!Faces || !Faces.createFace || !Images || !Images.createImage) {
                   throw new Error("Required helper functions (Faces/Images) are missing for face test setup.");
              }
  
              // Create 2 images
              const createdImage1 = await Images.createImage({ image_path: '/path/face_img1.jpg', original_width: 200, original_height: 200 });
              const createdImage2 = await Images.createImage({ image_path: '/path/face_img2.jpg', original_width: 200, original_height: 200 });
              const imgId1 = createdImage1.image_id;
              const imgId2 = createdImage2.image_id;
              faceTestData.imageIds.push(imgId1, imgId2);
  
              if (!imgId1 || !imgId2) throw new Error("Failed to create images for face tests.");
  
              // Manually create face records simulating detection results
              // Image 1: Person 1 (face A), Person 2 (face B)
              const faceA = await Faces.createFace({ image_id: imgId1, embedding: embeddingP1_1, bounding_box: bbox1 });
              const faceB = await Faces.createFace({ image_id: imgId1, embedding: embeddingP2_1, bounding_box: bbox2 });
              // Image 2: Person 1 (face C), Person 3 (face D)
              const faceC = await Faces.createFace({ image_id: imgId2, embedding: embeddingP1_2, bounding_box: bbox1 }); // Similar to face A
              const faceD = await Faces.createFace({ image_id: imgId2, embedding: embeddingP3_1, bounding_box: bbox3 });
  
              if (!faceA?.face_id || !faceB?.face_id || !faceC?.face_id || !faceD?.face_id) {
                  console.error("Face creation results:", { faceA, faceB, faceC, faceD });
                  throw new Error("Failed to create mock face data.");
              }
  
              // Store details for easy access in tests
              faceTestData.faceDetails[faceA.face_id] = { face_id: faceA.face_id, image_id: imgId1, embedding: embeddingP1_1, bbox: bbox1, personId: 1 };
              faceTestData.faceDetails[faceB.face_id] = { face_id: faceB.face_id, image_id: imgId1, embedding: embeddingP2_1, bbox: bbox2, personId: 2 };
              faceTestData.faceDetails[faceC.face_id] = { face_id: faceC.face_id, image_id: imgId2, embedding: embeddingP1_2, bbox: bbox1, personId: 1 };
              faceTestData.faceDetails[faceD.face_id] = { face_id: faceD.face_id, image_id: imgId2, embedding: embeddingP3_1, bbox: bbox3, personId: 3 };
          });
  
          afterAll(async () => {
              // Clean up faces and images
              for (const faceId in faceTestData.faceDetails) {
                  if (Faces?.deleteFaceById) await Faces.deleteFaceById(parseInt(faceId));
              }
          ...
          describe("GET /faces/:faceId", () => {
              ...
              beforeAll(() => {
                  ...
                  testFaceId = Object.values(faceTestData.faceDetails).find(f => f.personId === 1 && f.image_id === faceTestData.imageIds[0])?.face_id;
                  if (!testFaceId) throw new Error ("Could not find testFaceId for GET /faces/:faceId tests");
              });
              ...
              test("should retrieve a specific face by ID", async () => {
                  ...
                  expect(res.body.data).toHaveProperty("image_id", faceTestData.imageIds[0]); // Image 1
              ...
              test("should return 400 for invalid face ID format", async () => {
                  ...
                  const res = await agent
                      ...
                      .set("Authorization", `Bearer ${authToken}`);
                  expect(res.status).toBe(HTTP_STATUS_CODES.BAD_REQUEST);
                  expect(res.body.message).toMatch(/Invalid face ID format|Validation failed/i);
              });
          });
          ...
          describe("POST /faces/search", () => {
              ...
              beforeAll(() => {
                  ...
                  faceIdP1_Img1 = Object.values(faceTestData.faceDetails).find(f => f.personId === 1 && f.image_id === faceTestData.imageIds[0])?.face_id;
                  faceIdP2_Img1 = Object.values(faceTestData.faceDetails).find(f => f.personId === 2)?.face_id;
                  faceIdP1_Img2 = Object.values(faceTestData.faceDetails).find(f => f.personId === 1 && f.image_id === faceTestData.imageIds[1])?.face_id;
                  faceIdP3_Img2 = Object.values(faceTestData.faceDetails).find(f => f.personId === 3)?.face_id;
  
                  if (!faceIdP1_Img1 || !faceIdP1_Img2 || !faceIdP2_Img1 || !faceIdP3_Img2) {
                      throw new Error("Could not find all necessary face IDs for search tests");
                  }
              });
              ...
              test("should find similar faces based on faceId", async () => {
                  ...
                  expect(match).toHaveProperty("image_id", faceTestData.faceDetails[faceIdP1_Img2].image_id); // Image 2
