...
*   **Future Enhancements:**
    *   **Distributed Tracing (Tempo):** Trace requests across service boundaries (API -> Worker -> AI) to identify latency bottlenecks.
    *   **Structured JSON Logging:** Shift from plain text to JSON logs for advanced filtering and dashboarding in Grafana.
    *   **Application-Level Metrics:** Track business-specific metrics like queue lengths, match accuracy, and user activity.
    *   **Infrastructure Exporters:** Add dedicated exporters for PostgreSQL and Redis to monitor database health and connection pools.
