...
CREATE TABLE images(image_id UUID PRIMARY KEY,image_path TEXT NOT NULL,upload_time TIMESTAMP WITH TIME ZONE DEFAULT CURRENT_TIMESTAMP,creation_date TIMESTAMPTZ DEFAULT CURRENT_TIMESTAMP,update_date TIMESTAMPTZ DEFAULT CURRENT_TIMESTAMP);

CREATE TABLE faces(face_id SERIAL PRIMARY KEY,image_id UUID REFERENCES images(image_id),embedding REAL[],-- Store face embedding as a real array (adjust type based on embedding vector length)
 bounding_box JSONB,-- Store bounding box coordinates as JSON [top, right, bottom, left]
...
CREATE TABLE users(user_id UUID PRIMARY KEY,email TEXT NOT NULL,password TEXT NOT NULL);
